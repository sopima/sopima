...
$errorMsg = ['1'=>'Upload fehlgeschlagen.','2'=>'Dateityp nicht erlaubt.','3'=>'Datei zu groß (max. 20MB).'];
...
<div class="page-header">
    ...
    <div style="display:flex;gap:.5rem;">
        <a href="/contracts?action=edit&id=<?php echo $contract['id']; ?>" class="btn btn-outline"><i class="ti ti-edit"></i> Bearbeiten</a>
...
if ($contract["notice_date"]) {
    ...
    if ($days < 0) {
        $kbox = ['color'=>'#f87171','bg'=>'rgba(248,113,113,.08)','border'=>'rgba(248,113,113,.25)','icon'=>'ti-alert-circle',
            'text'=>'Kündigungsfrist abgelaufen (vor '.abs($days).' Tagen)',
            'sub'=>'Kündigung war bis '.date("d.m.Y",$notice_ts).' möglich.'];
    } elseif ($days === 0) {
        $kbox = ['color'=>'#f87171','bg'=>'rgba(248,113,113,.08)','border'=>'rgba(248,113,113,.25)','icon'=>'ti-alert-triangle',
            'text'=>'Kündigungsfrist endet heute!','sub'=>'Letzter Tag zur Kündigung: '.date("d.m.Y",$notice_ts)];
    } elseif ($days <= 30) {
        $kbox = ['color'=>'#f87171','bg'=>'rgba(248,113,113,.08)','border'=>'rgba(248,113,113,.25)','icon'=>'ti-alarm',
            'text'=>'Kündigung in '.$days.' Tagen möglich','sub'=>'Kündigungsfrist läuft am '.date("d.m.Y",$notice_ts).' ab.'];
    } elseif ($days <= 60) {
...
// Erweiterte Felder
$typeLabels       = ['versicherung'=>'Versicherung','mobilfunk'=>'Mobilfunk','internet'=>'Internet','darlehen'=>'Darlehen','abo'=>'Abonnement','wartung'=>'Wartung / Handwerker','sonstig'=>'Sonstig'];
$paymentLabels    = ['sepa'=>'SEPA-Lastschrift','ueberweisung'=>'Überweisung','kreditkarte'=>'Kreditkarte','bar'=>'Bar','paypal'=>'PayPal'];
$counterpartyLabels = ['unternehmen'=>'Unternehmen','privatperson'=>'Privatperson','behoerde'=>'Behörde'];
$extFields = [];
if ($contract['contract_type'])          $extFields[] = ['label'=>'Vertragstyp',              'value'=>$typeLabels[$contract['contract_type']] ?? $contract['contract_type']];
if ($contract['counterparty_type'])      $extFields[] = ['label'=>'Vertragspartner-Typ',       'value'=>$counterpartyLabels[$contract['counterparty_type']] ?? $contract['counterparty_type']];
if ($contract['payment_method'])         $extFields[] = ['label'=>'Zahlungsart',               'value'=>$paymentLabels[$contract['payment_method']] ?? $contract['payment_method']];
if ($contract['iban'])                   $extFields[] = ['label'=>'IBAN',                      'value'=>$contract['iban'], 'mono'=>true];
if ($contract['mandate_reference'])      $extFields[] = ['label'=>'SEPA-Mandatsreferenz',      'value'=>$contract['mandate_reference']];
if ($contract['minimum_term_months'])     $extFields[] = ['label'=>'Mindestlaufzeit',          'value'=>$contract['minimum_term_months'].' Monate'];
if ($contract['renewal_interval_months']) $extFields[] = ['label'=>'Verlängerungsintervall',   'value'=>$contract['renewal_interval_months'].' Monat(e)'];
if ($contract['auto_renewal'])           $extFields[] = ['label'=>'Automatische Verlängerung', 'value'=>'Ja'];
if ($contract['cancellation_period_days']) $extFields[] = ['label'=>'Kündigungsfrist',         'value'=>$contract['cancellation_period_days'].' Tage'];
if ($contract['cancellation_deadline'])  $extFields[] = ['label'=>'Nächstes Kündigungsdatum',  'value'=>date('d.m.Y', strtotime($contract['cancellation_deadline']))];
if ($contract['loan_amount'])            $extFields[] = ['label'=>'Darlehensbetrag',           'value'=>number_format($contract['loan_amount'],2,',','.').' €'];
if ($contract['interest_rate'])          $extFields[] = ['label'=>'Zinssatz',                  'value'=>number_format($contract['interest_rate'],2,',','.').' %'];
if ($contract['monthly_rate'])           $extFields[] = ['label'=>'Monatliche Rate',           'value'=>number_format($contract['monthly_rate'],2,',','.').' €'];
if ($contract['deductible'])             $extFields[] = ['label'=>'Selbstbeteiligung',         'value'=>number_format($contract['deductible'],2,',','.').' €'];
if ($contract['service_interval_months']) $extFields[] = ['label'=>'Wartungsintervall',        'value'=>$contract['service_interval_months'].' Monate'];
...
<div style="max-width:1100px;margin:0 auto 1.25rem;">
    <div style="display:flex;gap:.25rem;border-bottom:1px solid rgba(255,255,255,.1);padding-bottom:0;">
        <?php
        $tabs = [
            'uebersicht'   => ['icon'=>'ti-file-description', 'label'=>'Übersicht'],
            'details'      => ['icon'=>'ti-list-details',     'label'=>'Details',         'badge'=>$hasDetails ? null : null],
            'ansprechpartner' => ['icon'=>'ti-users',         'label'=>'Ansprechpartner', 'badge'=>$personsCount ?: null],
            'dokumente'    => ['icon'=>'ti-files',            'label'=>'Dokumente',       'badge'=>$docsCount ?: null],
        'kommunikation' => ['icon'=>'ti-message-dots',    'label'=>'Kommunikation',   'badge'=>$logCount ?: null],
        ];
...
<div style="max-width:1100px;margin:0 auto 1rem;padding:1rem 1.25rem;background:<?php echo $kbox['bg']; ?>;border:1px solid <?php echo $kbox['border']; ?>;border-radius:12px;display:flex;align-items:center;gap:1rem;">
    ...
    <div>
        ...
        <div style="font-size:.83rem;color:var(--text-muted);"><?php echo $kbox['sub']; ?>
            ...
            &nbsp;·&nbsp; Nächste Verlängerung: <strong style="color:rgba(255,255,255,.6);"><?php echo $kbox['renewal']; ?></strong>
            <?php endif; ?>
            <?php if (!empty($kbox['kuendigung_heute'])): ?>
            &nbsp;·&nbsp; Bei Kündigung heute: Vertragsende <strong style="color:rgba(255,255,255,.6);"><?php echo $kbox['kuendigung_heute']; ?></strong>
            <?php endif; ?>
        </div>
    </div>
</div>
...
<div class="card" style="padding:1.25rem 1.5rem;max-width:1100px;margin:0 auto 1rem;">
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:.75rem 1.5rem;">
        <div>
            <div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Vertragsnummer</div>
            <div style="font-size:.9rem;font-family:monospace;color:#a5b4fc;"><?php echo htmlspecialchars($contract['contract_number'] ?? '–'); ?></div>
        </div>
        <div>
            <div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Mandant</div>
            <div style="font-size:.9rem;"><?php echo htmlspecialchars($contract['client_name']); ?></div>
        </div>
        <div>
            <div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Partner</div>
            <div style="font-size:.9rem;"><?php echo htmlspecialchars($contract['partner'] ?? '–'); ?></div>
        </div>
        <div>
            <div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Status</div>
            <div style="font-size:.9rem;color:<?php echo $color; ?>;"><?php echo ucfirst($contract['status']); ?></div>
        </div>
        <div>
            <div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Startdatum</div>
            <div style="font-size:.9rem;"><?php echo $contract['start_date'] ? date('d.m.Y', strtotime($contract['start_date'])) : '–'; ?></div>
        </div>
        <div>
            <div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Enddatum</div>
            <div style="font-size:.9rem;"><?php echo $contract['end_date'] ? date('d.m.Y', strtotime($contract['end_date'])) : '–'; ?></div>
        </div>
        <div>
            <div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Kündigungsdatum</div>
            <div style="font-size:.9rem;"><?php echo $contract['notice_date'] ? date('d.m.Y', strtotime($contract['notice_date'])) : '–'; ?></div>
        </div>
        <div>
            <div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Wert</div>
            <div style="font-size:.9rem;"><?php echo $contract['value'] ? number_format($contract['value'],2,',','.').' €' : '–'; ?></div>
        </div>
        <div>
            <div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Intervall</div>
        ...
        <div style="grid-column:span 3;">
            <div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Notizen</div>
...
<div class="card" style="padding:1.25rem 1.5rem;max-width:1100px;margin:0 auto 1rem;">
    <div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.75rem;">Eigene Felder</div>
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:.75rem 1.5rem;">
...
<div class="card" style="padding:1.25rem 1.5rem;max-width:1100px;margin:0 auto 1rem;">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:.75rem 1.5rem;">
        ...
        <div><div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Firma</div><div style="font-size:.9rem;"><?php echo htmlspecialchars($contact['company']); ?></div></div>
        <?php endif; ?>
        <?php if ($contact['first_name'] || $contact['last_name']): ?>
        <div><div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Name</div><div style="font-size:.9rem;"><?php echo htmlspecialchars(trim(($contact['first_name'] ?? '').' '.($contact['last_name'] ?? ''))); ?></div></div>
        <?php endif; ?>
        <?php if ($contact['email']): ?>
        <div><div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">E-Mail</div><div style="font-size:.9rem;"><a href="mailto:<?php echo htmlspecialchars($contact['email']); ?>" style="color:#93c5fd;"><?php echo htmlspecialchars($contact['email']); ?></a></div></div>
        <?php endif; ?>
        <?php if ($contact['phone']): ?>
        <div><div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Telefon</div><div style="font-size:.9rem;"><?php echo htmlspecialchars($contact['phone']); ?></div></div>
        <?php endif; ?>
        <?php if ($contact['mobile']): ?>
        <div><div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Mobil</div><div style="font-size:.9rem;"><?php echo htmlspecialchars($contact['mobile']); ?></div></div>
        <?php endif; ?>
        <?php if ($contact['street'] || $contact['city']): ?>
        <div><div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Adresse</div><div style="font-size:.9rem;"><?php echo htmlspecialchars($contact['street'] ?? ''); ?><br><?php echo htmlspecialchars(trim(($contact['zip'] ?? '').' '.($contact['city'] ?? ''))); ?></div></div>
        <?php endif; ?>
        <?php if ($contact['iban']): ?>
        <div><div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">IBAN</div><div style="font-size:.9rem;font-family:monospace;"><?php echo htmlspecialchars($contact['iban']); ?></div></div>
        <?php endif; ?>
        <?php if ($contact['bank']): ?>
        <div><div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">Bank</div><div style="font-size:.9rem;"><?php echo htmlspecialchars($contact['bank']); ?></div></div>
        <?php endif; ?>
        <?php if ($contact['bic']): ?>
        <div><div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.2rem;">BIC</div><div style="font-size:.9rem;font-family:monospace;"><?php echo htmlspecialchars($contact['bic']); ?></div></div>
        <?php endif; ?>
    </div>
</div>
<?php endif; ?>
...
<div class="card" style="padding:2.5rem 1.5rem;max-width:1100px;margin:0 auto;text-align:center;color:var(--text-muted);font-size:.85rem;">
    ...
    Keine erweiterten Felder vorhanden.
</div>
<?php endif; ?>
...
<div class="card" style="max-width:1100px;margin:0 auto;">
    ...
    <div style="padding:1rem 1.25rem;">
        <form method="POST" action="/contracts?action=upload" enctype="multipart/form-data" style="display:grid;grid-template-columns:1fr auto auto;gap:.6rem;align-items:end;margin-bottom:1rem;">
            ...
            <button type="submit" class="btn btn-primary" style="white-space:nowrap;"><i class="ti ti-upload"></i> Hochladen</button>
        </form>
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid rgba(255,255,255,.05);">
                    ...
                    <td style="padding:.6rem .4rem;text-align:right;">
                        <form method="POST" action="/contracts?action=delete_doc&tab=dokumente" onsubmit="return confirm('Dokument löschen?')">
...
<div style="max-width:1100px;margin:0 auto 1rem;">
    ...
    <div class="card" style="padding:1.25rem 1.5rem;margin-bottom:1rem;">
        <div style="font-size:.72rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.75rem;">Neuer Eintrag</div>
        <form method="POST" action="/contracts?action=view&id=<?php echo $contract['id']; ?>&tab=kommunikation"
            ...
            <div class="form-group" style="margin:0;grid-column:span 4;">
                <label style="font-size:.72rem;">Betreff *</label>
                <input type="text" name="subject" required placeholder="z.B. Kündigung telefonisch bestätigt">
            </div>
            <div class="form-group" style="margin:0;">
                <label style="font-size:.72rem;">Kanal</label>
                <select name="channel">
            ...
            <div class="form-group" style="margin:0;">
                <label style="font-size:.72rem;">Richtung</label>
            ...
            <div class="form-group" style="margin:0;">
                <label style="font-size:.72rem;">Datum & Uhrzeit</label>
                <input type="datetime-local" name="logged_at" value="<?php echo date('Y-m-d\TH:i'); ?>">
            </div>
            <div class="form-group" style="margin:0;display:flex;align-items:flex-end;">
                <button type="submit" class="btn btn-primary" style="width:100%;"><i class="ti ti-plus"></i> Eintragen</button>
            </div>
            <div class="form-group" style="margin:0;grid-column:span 4;">
                <label style="font-size:.72rem;">Notiz</label>
                <textarea name="body" rows="2" placeholder="Gesprächsinhalt, Ergebnis, nächste Schritte…" style="resize:vertical;"></textarea>
            </div>
        </form>
    </div>
    ...
    <div class="card" style="padding:1.25rem 1.5rem;">
        ...
        $channelLabels = ['telefon'=>'Telefon','email'=>'E-Mail','brief'=>'Brief',
                          'portal'=>'Portal','persoenlich'=>'Persönlich','sonstig'=>'Sonstig'];
        ...
        <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:8px;padding:.75rem 1rem;">
            <div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.3rem;flex-wrap:wrap;">
                ...
                <span style="font-size:.75rem;padding:.1rem .5rem;border-radius:20px;background:rgba(255,255,255,.06);color:<?php echo $directionColors[$entry['direction']]; ?>;">
                    <?php echo ucfirst($entry['direction']); ?>
                </span>
                ...
                <form method="POST" action="/contracts?action=view&id=<?php echo $contract['id']; ?>&tab=kommunikation_delete"
                    ...
                    <button type="submit" style="background:none;border:none;color:#f87171;cursor:pointer;padding:.1rem .3rem;" title="Löschen">

...
?>
<?php if ($saved === '1'): ?>
<div class="alert alert-success" style="margin-bottom:1rem;padding:.75rem 1rem;background:var(--success-bg);border:1px solid #bbf7d0;border-radius:7px;color:var(--success);font-size:.88rem;">
...
<div style="margin-bottom:1rem;padding:.65rem 1rem;background:#eef2ff;border:1px solid #c7d2fe;border-radius:7px;font-size:.83rem;color:#3a5bd4;display:flex;align-items:center;gap:.5rem;">
    ...
    <?php echo __('pdf_templates.hint'); ?>
</div>
    ...
    <form id="pdf-form-<?php echo $tpl['id']; ?>" method="POST" action="/settings?tab=pdf&action=save" enctype="multipart/form-data" style="padding:1.25rem 1.5rem;display:flex;flex-direction:column;gap:1rem;">
        ...
        <div>
            ...
            <div style="display:flex;gap:.3rem;margin-bottom:.3rem;flex-wrap:wrap;padding:.4rem;background:#f9fafb;border:1px solid var(--border);border-bottom:none;border-radius:6px 6px 0 0;">
                <button type="button" onclick="pdfFmt(<?php echo $tpl['id']; ?>,'bold')" style="padding:.2rem .55rem;font-weight:700;border:1px solid var(--border);background:#fff;color:var(--text);border-radius:3px;cursor:pointer">B</button>
                <button type="button" onclick="pdfFmt(<?php echo $tpl['id']; ?>,'italic')" style="padding:.2rem .55rem;font-style:italic;border:1px solid var(--border);background:#fff;color:var(--text);border-radius:3px;cursor:pointer">I</button>
                <button type="button" onclick="pdfFmt(<?php echo $tpl['id']; ?>,'underline')" style="padding:.2rem .55rem;text-decoration:underline;border:1px solid var(--border);background:#fff;color:var(--text);border-radius:3px;cursor:pointer">U</button>
                <span style="border-left:1px solid var(--border);margin:0 .2rem"></span>
                <button type="button" onclick="pdfFmt(<?php echo $tpl['id']; ?>,'justifyLeft')" title="Linksbündig" style="padding:.2rem .55rem;border:1px solid var(--border);background:#fff;color:var(--text);border-radius:3px;cursor:pointer">&#8676;</button>
                <button type="button" onclick="pdfFmt(<?php echo $tpl['id']; ?>,'justifyCenter')" title="Zentriert" style="padding:.2rem .55rem;border:1px solid var(--border);background:#fff;color:var(--text);border-radius:3px;cursor:pointer">&#8596;</button>
                <button type="button" onclick="pdfFmt(<?php echo $tpl['id']; ?>,'justifyRight')" title="Rechtsbündig" style="padding:.2rem .55rem;border:1px solid var(--border);background:#fff;color:var(--text);border-radius:3px;cursor:pointer">&#8677;</button>
                <span style="border-left:1px solid var(--border);margin:0 .2rem"></span>
                <select onchange="pdfFmtSize(<?php echo $tpl['id']; ?>,this.value);this.selectedIndex=0;" style="padding:.2rem .4rem;background:#fff;border:1px solid var(--border);color:var(--text);border-radius:3px;font-size:.8rem;">
                ...
                <span style="border-left:1px solid var(--border);margin:0 .2rem"></span>
                <select onchange="pdfInsertPH(<?php echo $tpl['id']; ?>,this.value);this.selectedIndex=0;" style="padding:.2rem .4rem;background:#fff;border:1px solid var(--border);color:var(--text);border-radius:3px;font-size:.8rem;">
            ...
            <div id="pdf-editor-<?php echo $tpl['id']; ?>"
                 ...
                 style="min-height:280px;background:#fff;color:#111;border:1px solid var(--border);border-radius:0 0 6px 6px;padding:.75rem;font-size:11pt;line-height:1.7;font-family:Arial,Helvetica,sans-serif;overflow-y:auto;"
